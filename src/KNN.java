...
public class KNN {
    ...
    public Map<String, List<Double>> getMinMax(LoadData loadData) {
        ...
        List<String> nameList = loadData.getNameList();
        String targetType = nameList.get(nameList.size() - 1);
        for (Map.Entry<String, AttributeSet> entry : attributeSetMap.entrySet()) {
            //only get Max & Min for attributes
            if (!targetType.equals(entry.getKey())) {
...
//                    System.out.println(key);
                    ...
                    res += (1 - calMatrix[data1_index][data2_index]) * weight;
//                    System.out.println(res);
    ...
    public double calClass(PriorityQueue<DataSet> pq, String target, int k) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            res += dataSet.getValue(target) * dataSet.getSimilarity();
            sumSimilarity += dataSet.getSimilarity();
        }
        return res / sumSimilarity;
    }
    ...
    public String getClass(PriorityQueue<DataSet> pq, Map<String, Double> map, String target, int k) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            for (Map.Entry<String, Double> entry : map.entrySet()) {
                if (entry.getValue().equals(dataSet.getValue(target))) {
...
//                    System.out.println(classType);
        ...
        list.sort(((o1, o2) -> o2.getValue() - o1.getValue() > 0 ? 1 : -1));
    ...
    public String KNN(LoadData train, DataSet testSet, Map<String, Double> weightMap, Map<String, double[][]> marixMap, int k) {
        String tagetType = train.getNameList().get(train.getNameList().size() - 1); // set target type="label"
        PriorityQueue<DataSet> pq = new PriorityQueue<>(k, (o1, o2) -> o1.getSimilarity() > o2.getSimilarity() ? 1 : -1);
